...
//            ImageUtil.saveImage(file,resume.getPhoto(),"companyIcon");
    ...
    @RequiresUser
    @PostMapping("/resume/delivery/{id}")
    public Result delivery(@PathVariable("id")String id) throws ServiceException {
        User user = (User) SecurityUtils.getSubject().getPrincipal();
//        Resume resume = resumeService.addDelivery(user.getId(),id);
//        if(ObjectUtil.isNull(resume)){
//            throw new ServiceException("简历投递失败!");
//        }
